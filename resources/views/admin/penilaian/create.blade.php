        ...
        <form action="{{ route('admin.penilaian.store') }}" method="POST">
            ...
            <div class="mb-4 d-flex align-items-start gap-2">
                ...
                <div class="d-flex flex-row gap-2">
                    {{-- DI SINI BIASANYA SELECT DARI TABEL PENGGUNA --}}
                    <input type="number" name="id_pengguna" id="id_pengguna" class="form-control @error('id_pengguna') is-invalid @enderror"
                        placeholder="Masukkan ID Pengguna" value="{{ old('id_pengguna') }}" style="width:500px;" required>
            ...
            <div class="mb-4 d-flex align-items-start gap-2">
                ...
                <div class="d-flex flex-row gap-2">
                    {{-- DI SINI BIASANYA SELECT DARI TABEL SKORING --}}
                    <input type="number" name="id_skoring" id="id_skoring" class="form-control @error('id_skoring') is-invalid @enderror"
                        placeholder="Masukkan ID Skoring" value="{{ old('id_skoring') }}" style="width:500px;" required>
                    <span class="text-danger">*</span>
                </div>
                @error('id_skoring')
                    <div class="invalid-feedback d-block ms-2">{{ $message }}</div>
