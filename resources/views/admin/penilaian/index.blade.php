...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div>
        <a href="{{ route('admin.penilaian.create') }}" class="btn btn-success">
            <i class="ri-add-line"></i> Tambah Penilaian
        </a>
    </div>
</div>
                ...
                <tbody>
                    @foreach($penilaian as $index => $item)
                        <tr>
                            ...
                            <td>{{ $item->id_skoring }}</td>
                            <td class="text-center">{{ $item->bobot }}</td>
                            <td class="text-center">{{ $item->skor }}</td>
                            <td class="text-center">{{ $item->nilai }}</td>
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="{{ route('admin.penilaian.edit', $item->id) }}" class="btn btn-sm btn-primary">
                                        <i class="ri-edit-line"></i> Edit
                                    </a>

                                    <form action="{{ route('admin.penilaian.destroy', $item->id) }}" method="POST" class="form-hapus" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
